...
class Assistants extends AdminController
{
    public function index()
    {
        ...
        if (!empty($_GET['id']) && is_string($_GET['id'])) {
            $result = DB::select('chat_assistants', '*', ['id' => base64_decode($_GET['id']), 'user_id' => 0]);
            $assistant = isset($result[0]) ? $result[0] : null;
            if (empty($assistant)) $this->redirect("admin/assistants");
            ...
            if (isset($_POST['delete_assistant']) && $_POST['delete_assistant'] == md5($assistant['id'])) {
                ...
                $this->redirect("admin/assistants", "Assistant has been deleted successfully.");
            }
        }
        ...
        if (isset($_FILES['files'])) {
            ...
            if (empty($assistantResult[0])) {
                // get default model
                $setting = $this->setting(['name' => 'default_chat_model']);
                $model = !empty($setting['default_chat_model']) ? $setting['default_chat_model'] : 'gpt-4o-mini';
                // create assistants
                $assistantData = [
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $id = base64_encode($id);
            if (!empty($assistant['id'])) $this->redirect("admin/assistants?id=$id", "Your details were changed successfully.");
            $this->redirect("admin/assistants?id=$id", "Your details were added successfully.");
        }

        $limit = 50;
        $page = !empty($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
        $offset = ($page * $limit) - $limit;
        $assistants = DB::select('chat_assistants', '*', ['user_id' => 0], 'ORDER BY name ASC LIMIT ' . $offset . ',' . $limit);
        $pager = new Pagination($page, $limit);
        $total = DB::count('chat_assistants', ['user_id' => 0]);
        $pager->total($total);
        $pagination = $pager->execute(true);
        $models = DB::select('models', '*', ['type' => 'GPT', 'user_id' => 0]);

        $this->title('Assistants');
        require_once APP . '/View/Admin/Assistants.php';
    }
