...
class MyTemplates extends UserController
{
    public function index()
    {
        ...
        // allow to create template
        $totalMyTemplates = DB::count('templates', ['user_id' => $this->uid]);
        if (!empty($this->plan['my_template']) && (is_null($this->plan['total_templates']) || $this->plan['total_templates'] > $totalMyTemplates)) {
            $template_access = true;
        }

        if (isset($_GET['create']) && empty($template_access)) {
            $this->redirect("my/templates/list");
        }

        if (isset($_GET['id']) && is_numeric($_GET['id'])) {
            $template_result = DB::select('templates', '*', ['id' => $_GET['id'], 'user_id' => $this->uid], 'LIMIT 1');
            $template = isset($template_result[0]) ? $template_result[0] : null;
            if (!$template) $this->redirect("my/templates/list");
            ...
            if (!empty($_GET['duplicate']) && $_GET['duplicate'] == md5($template['id'])) {
                if (empty($template_access)) $this->redirect("my/templates/list");
                ...
                if (!empty($results[0])) {
                    ...
                    if ($id) $this->redirect("my/templates/list?id=$id", "A new template has been successfully created. You can now modify it to meet your needs.");
                }
                $this->redirect("my/templates/list", "The system failed to create a duplicate template.", "error");
            }
            ...
            if (!empty($_POST['delete_template']) && $_POST['delete_template'] == md5($template['id'])) {
                ...
                $this->redirect("my/templates/list", "Templates has been deleted successfully.");
            }
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['title'])) {
            ...
            if ($id) $this->redirect("my/templates/list?id=$id", "Template has been created successfully.");
            $this->redirect("my/templates/list", "Something went wrong, please try again.", "error");
        }

        $models = DB::query("SELECT * FROM models WHERE type = 'GPT' AND (user_id = 0 OR user_id = $this->uid)");
        $total = DB::count('templates', ['user_id' => $this->uid]);
        $result_templates = DB::query("select templates.*, models.name as model_name From templates LEFT JOIN models ON templates.model=models.id WHERE templates.user_id=$this->uid ORDER BY templates.group_name ASC, templates.title ASC");
        $group_templates = [];
        ...
        require_once APP . '/View/User/MyTemplates.php';
    }
