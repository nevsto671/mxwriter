                ...
                <div class="col-sm-12 col-lg-5 mb-3 mb-md-0">
                    <div class="mx-auto" style="max-width: 320px">
                        ...
                        <?php if ($flash = $this->flash()) { ?>
                            <div class="p-2 text-center alert alert-<?php echo $flash['type']; ?>"><?php echo $flash['message']; ?></div>
                        <?php } ?>
                        <form method="post" id="device-verification-form" style="display: none">
                            <p class="text-muted">
                                <b>Device verification</b><br>We need you to verify your identity before you log in. We just sent your verification code via email. Please check your email and enter the verification code below.
                            </p>
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Code</label>
                                <input type="tel" name="verification_code" class="form-control" placeholder="Verification code" maxlength="12" required>
                            </div>
                            <div class="text-center text-danger small error-message mb-2"></div>
                            <div class="mb-3">
                                <button type="submit" class="btn btn-primary py-2 w-100" id="device-verification-submit">Verify</button>
                            </div>
                            <div class="mb-2 text-center resend-code">
                                <button type="button" class="btn btn-link btn-resend p-0" style="display: none;">Resend OTP</button>
                            </div>
                        </form>
                        <div id="email-login" data-redirect="<?php echo $this->url(isset($_GET['redirect']) ? $_GET['redirect'] : 'my'); ?>">
                            ...
                            <form method="post" id="email-login-form">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="password" name="password" class="form-control" placeholder="Your account password" autocomplete="current-password" required>
                                </div>
                                <div class="text-center text-danger small error-message mb-2"></div>
                                <div class="mb-3">
                                    <button type="submit" class="btn btn-primary py-2 w-100" id="email-login-submit" data-loader="Login...">Continue</button>
                                </div>
                                <div class="text-center">
                                    <div class="mb-4">
                                        <a href="<?php echo $this->url('reset'); ?>" class="text-decoration-none">Forgot password?</a>
                                    </div>
                                    <?php if ($setting['registration_status']) { ?>
                                        <div class="fw-semibold">Don't have an account? <a href="<?php echo $this->url('signup'); ?>" class="text-decoration-none">Sign Up</a></div>
                                    <?php } ?>
                                </div>
                            </form>
                        </div>
